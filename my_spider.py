...
class MyRobot(threading.Thread):
	...
	def db_save(self, data):
		...
		c = conn.cursor()
		sql = "INSERT INTO articles (title, content, article_id, url_from, is_published, articleType_id, create_time, update_time, source_id, num_of_view) VALUES ('%s','%s', %s, '%s', 1, 1, '%s', '%s', 1, 1)"%(data['title'], data['content'], data['article_id'], data['url'], time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
